...
<html lang="en-US">
	...
	FROM `brief_summary`
	INNER JOIN `dates`
	ON brief_summary.ctri_number = dates.ctri_number
	INNER JOIN `study_details` 
	ON brief_summary.ctri_number = study_details.ctri_number
        INNER JOIN `study_name` 
	ON brief_summary.ctri_number = study_name.ctri_number
        INNER JOIN `health_condition` 
	ON brief_summary.ctri_number = health_condition.ctri_number
        INNER JOIN `intervention_comparator` 
	ON brief_summary.ctri_number = intervention_comparator.ctri_number
	WHERE 
	LOWER(brief_summary.ctri_number) LIKE LOWER('%$value_ctri%') 
	AND 
	LOWER(brief_summary.ctri_number) LIKE LOWER('%%') 
	AND 
	dates.registered_on LIKE '%$value_regdate%' 
	AND 
	LOWER(study_name.scientific_title) LIKE LOWER('%$value_scientific_title%') 
	AND 
	LOWER(health_condition.condition) LIKE LOWER('%$value_condition%') 
	AND 
	study_details.type_of_trial LIKE '%$value_trial_type%' 
	AND 
	study_details.phase LIKE '%$value_phase%' 
	AND 
	LOWER(intervention_comparator.comparator_name) LIKE LOWER('%$value_comparator_name%') 
	AND 
	LOWER(intervention_comparator.intervention_name) LIKE LOWER('%$value_intervention_name%');";
